#!/usr/bin/env python3
"""
Boxercise 30分動画生成スクリプト v2（新ロゴ仕様 / 標準仕様v1準拠）
- 基準実装: make_cafe_warm_30min.py (cafe_warm_30min_004)
- 右上ロゴ: latte_dog_icon_circle_v2.png（新チャンネルアイコン案Bベース）+ LATTE BGM 文字（別レイヤー）
- Ken Burns / 照明揺らぎは旧 boxercise プリセット値を踏襲
- 既存 boxercise_30min_001.mp4 は残し、_002 として新版を出力
- 使い方: python3 make_boxercise_30min_v2.py [--test]   # --test で10秒テストレンダー
"""
...
IMAGE  = f"{ROOT}/assets/latte_bgm/images/organized/04_Boxercise/boxercise_005.png"
AUDIO  = f"{ROOT}/assets/latte_bgm/audio/source/workout_boxercise_001.mp3"
...
OUTPUT = (f"{ROOT}/assets/latte_bgm/videos/test/boxercise_30min_002_test10s.mp4"
          if TEST else
          f"{ROOT}/assets/latte_bgm/videos/final/boxercise_30min_002.mp4")
...
# ── Ken Burns (workout_boxercise プリセット) ────────────────
z_start, z_end = 1.00, 1.16
z_rng = z_end - z_start
...
# ── ジム照明フリッカー (旧 boxercise 値) ───────────────────
brightness, contrast, saturation = 0.02, 1.08, 1.10
flicker_speed, flicker_amp = 1.6, 0.035
...
print("=" * 58)
print("  Latte BGM — Boxercise 30min v2 (新ロゴ仕様)" + ("  [TEST 10s]" if TEST else ""))
print("=" * 58)
print(f"  画像  : {IMAGE}")
print(f"  音源  : {AUDIO}")
print(f"  アイコン: {ICON}")
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    cmd = [
        ...
        "-r", "30", "-t", str(D),
        "-movflags", "+faststart",
        "-metadata", "title=30 Min Boxercise Workout Music | Fitness Motivation BGM | Latte BGM",
        OUTPUT,
    ]
